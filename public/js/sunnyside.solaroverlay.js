...
SolarOverlay.prototype.initValues = function () {
    var timeSelect = this.timeSelect_,
        raysContainer = this.raysContainer_,
        selectedHour = this.timeSelect_.val();
    ...
    var currDate = new Date(),
        ...
        hour = currMin > 30 ? currHour + 1 : currHour;

    if (selectedHour !== hour && selectedHour !== null) hour = selectedHour;

    var minHour = 24, maxHour = 0;

    jQuery.each(this.solarpositions_, function (index, position) {
        minHour = position.hour < minHour ? position.hour : minHour;
        maxHour = position.hour > maxHour ? position.hour : maxHour;
    ...
    if ($('.sunray.active').length == 0) {
        if (Math.abs(minHour - hour) < Math.abs(maxHour - hour)) {
            $('.sunray').first().addClass('active');
        } else {
            $('.sunray').last().addClass('active');
            this.timeSelect_.val(maxHour).trigger('change');
        }
    }
